...
- **Rendering technology family: SVG.** Resolves Open Decision #2 below (✅ validated by spike6, 2026-05-31). Canvas and WebGL ruled out at the 200-node target — SVG's DOM-text affordances are first-class for Cmd+K search and SVG/PNG/PDF export, and disclosure interactions reduce to attribute mutations.
...
- **Renderer technology** — ✅ **RESOLVED 2026-05-31, validated by spike6.** Locked as **SVG + parser-only + dagre + d3-shape** (see Architecture Decisions above). The validation spike (a representative flowchart with nested subgraphs + drag interaction) passed and went considerably further into Mermaid layout parity. *Scope:* this validated the technology choice, not the performance release gate — the 200/500/1000-node targets remain unmeasured (Phase-1 risk). As-built detail in `docs/architecture/`; closure in `spike6/SPIKE6_COMPLETE.md`.
