...
- **Mermaid subgraph-ordering parity:** `chooseEdgesToReverseForMermaidOrder(ir)` builds the inter-cluster adjacency between top-level subgraphs, runs Tarjan's SCC, and picks a feedback arc set per SCC using this priority:
  1. **If the SCC contains dotted edges (`-.->`)**, reverse only those. Mermaid treats dotted as the conventional "back-edge" marker in cyclic flows; preferring dotted matches Mermaid's resulting rank order on diagrams whose primary flow is one direction with informational back-edges (e.g. `fixture_crosscluster.mmd`: Frontend → Services → DataLayer → External plus two dotted back-edges).
  2. **Otherwise** reverse every earlier→later inter-cluster edge in the SCC. After reversal all surviving inter-cluster edges point later→earlier, which dagre ranks as "later above earlier" — matches Mermaid on solid-only cycles (e.g. `fixture.mmd`: Authentication ↔ Payment_System, Payment_System ends up above Authentication).

  Edge points are flipped back post-layout so rendering still sees the original direction. Edges outside any SCC are left untouched. The SCC approach replaced an earlier per-edge BFS that re-derived the same membership information O(V·E) times and didn't generalise to graphs with multiple overlapping cycles. Live-toggleable via `layoutSettings.mermaidParity` and the "Mermaid parity" button — toggle off to see the raw `@dagrejs/dagre` ordering that the fix is correcting.

  **Why this is needed (verified 2026-05-20):** It is *not* a difference between the two dagre forks. `@dagrejs/dagre` and `dagre-d3-es` (the fork Mermaid uses) implement the same DFS-based feedback-arc-set algorithm — given the same `graphlib.Graph` they produce the same reversed edges. The divergence comes from upstream of dagre: Mermaid pre-processes the graph in `mermaid-graphlib.js` before calling layout. Three steps matter:
    1. **`adjustClustersAndEdges`** rewrites every cross-cluster edge so its endpoint becomes the cluster id rather than the inner node.
    2. **`extractor`** lifts each self-contained subgraph into its own nested `graphlib.Graph` and runs dagre separately per nested graph (deepest first), then uses each result as the size of that subgraph's node in the parent layout.
    3. **`sortNodesByHierarchy`** re-emits nodes in parent-then-children order before each dagre call.

  `dfsFAS` iterates `g.nodes()` in insertion order and picks the first back-edge it hits per DFS branch. Mermaid's preprocessing changes both the edge set dagre sees and the node order it walks, which flips which back-edge gets reversed — which flips which subgraph ranks above.

  **Why we don't replicate the full pipeline:** It would be a ~500–1000-line port of Mermaid internals (PRD `architecture-decisions-renderer.md` Decision 3 explicitly chose against reverse-engineering Mermaid's renderer). It would also force our flat IR to become nested, which has knock-on costs for `effective-ir.ts`, A* routing, drag-pinning, and the surrogate model that Spike 3 deliberately kept simple.

  **Trigger to revisit:** if a second or third Mermaid-parity bug appears that can't be patched with a similar small adapter — i.e., we're accumulating Mermaid-shaped workarounds — re-evaluate porting the preprocessing pipeline (or just `adjustClustersAndEdges` as a verified spike). Until then, the small adapter is the right trade.
- **Branch-ordering correction.** On flat graphs (no subgraphs), `fixBranchOrdering` finds the first branching node and, if dagre placed its first-declared child to the right of its second-declared child, mirrors the entire graph horizontally around its x-midpoint. On graphs with subgraphs the global mirror is unsafe (it would also flip sibling-subgraph positions away from Mermaid's reference), so `fixBranchOrderingPerSubgraph` runs instead: for each top-level subgraph it finds the first branching node whose source and first two declared targets all sit inside that subgraph, and if the first-declared target landed to the right of the second-declared, mirrors that subgraph's interior nodes AND intra-subgraph edge waypoints around the local x-midpoint.

  **Critical constraint:** the per-subgraph mirror skips any cluster whose members participate in inter-cluster edges. Reason: dagre's waypoints for an inter-cluster edge are drawn against the pre-mirror interior node positions; mirroring the interior would leave those waypoints pointing at where nodes used to be, and even though `clipToBorder` re-anchors the endpoints, the middle waypoints still curve toward the old position, producing visibly kinked edges. Without this guard the mirror regresses `fixture.mmd` (every cluster there has at least one inter-cluster edge). The trade-off is that clusters connected via inter-cluster edges don't get mirror correction — accepted as cosmetic; long-term fix would require rebuilding affected inter-cluster edge waypoints.
...
| `chooseEdgesToReverseForMermaidOrder` is O(V + E) Tarjan SCC over the inter-cluster graph | Spike 4 (Bug 1 fix, revised) | Fine at 200 nodes. Required because we hand dagre a flat graph instead of replicating Mermaid's per-subgraph nested-layout pre-processing — see §6 for the full explanation. Replaces the original per-edge BFS that didn't generalise to multi-cycle topologies. |

---



**Read this whole doc, then `SPIKE_NOTES.md`, if you need to look at the code then read `spike4/src/` as the reference. Every file in §14 has a one-job contract**
