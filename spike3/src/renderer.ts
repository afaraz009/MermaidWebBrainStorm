...
// edge has been re-routed, else dagre's originalPoints. `mode` tells the path
// builder how to render the polyline — A* output is a corner polyline drawn as
// straight segments; dagre output is fed to curveBasis for a smooth curve.
...
export function renderFull(ir: IR, mountEl: SVGElement, interactive = false, originalIR?: IR): void {
  ...
  for (const e of ir.edges) {
    ...
    path.setAttribute('d', edgePathString({ pts, mode }));
  ...
  for (const n of ir.nodes) {
    ...
    if (surrogate) {
      // "Stacked card" shadow behind the main rect.
